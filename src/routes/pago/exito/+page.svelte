<script lang="ts">
	...
	// 6b). Clearing the cart is a UX convenience (their selection has been ordered),
	// not a payment signal.
	onMount(() => {
		cart.clear();
	});
</script>
...
<svelte:head>
	<title>{t('pago.success.heading', locale)} — {t('site.name', locale)}</title>
